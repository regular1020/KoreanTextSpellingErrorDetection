...
for line in kcc150:
    ...
    words = koreanLineWithSpace.split(" ")
    for word in words:
        m = p.match(word)
        if m:
            newWord, newFreq = editErrorNgram(m.group(), word, wordUnigram[word])
            if word != newWord:
                tmpList.append(word)
                tmpList.append(newWord)
                tmpList.append(line)
                tmpList.append("1")
                tmpList.append(str(wordUnigram[word]))
                tmpList.append(str(newFreq))
                errorList.append(tmpList)
                break

    if len(tmpList) < 1:
        for word in words:
            for i in range(len(word)):
                if syllableUnigram[word[i]] < 10:
                    newWord, newFreq = editErrorNgram(word[i], word, wordUnigram[word])
                    if word != newWord:
                        tmpList.append(word)
                        tmpList.append(newWord)
                        tmpList.append(line)
                        tmpList.append("2")
                        tmpList.append(str(syllableUnigram[word[i]]))
                        tmpList.append(str(newFreq))
                        errorList.append(tmpList)
                        break
            if len(tmpList) > 0:
        ...
        for word in words:
            if (len(word)) >= 2:
                for i in range(len(word)-2):
                    if syllableBigram[word[i:i+2]] < 5:
                        newWord, newFreq = editErrorNgram(word[i:i+2], word, wordUnigram[word])
                        if word != newWord:
                            tmpList.append(word)
                            tmpList.append(newWord)
                            tmpList.append(line)
                            tmpList.append("3")
                            tmpList.append(str(syllableBigram[word[i:i+2]]))
                            tmpList.append(str(newFreq))
                            errorList.append(tmpList)
                            break
            if len(tmpList) > 0:
        ...
        for word in words:
            if len(word) >= 3:
                for i in range(len(word)-3):
                    if syllableTrigram[word[i:i+3]] < 3:
                        newWord, newFreq = editErrorNgram(word[i:i+3], word, wordUnigram[word])
                        if word != newWord:
                            tmpList.append(word)
                            tmpList.append(newWord)
                            tmpList.append(line)
                            tmpList.append("4")
                            tmpList.append(str(syllableTrigram[word[i:i+3]]))
                            tmpList.append(str(newFreq))
                            errorList.append(tmpList)
                            break
            if len(tmpList) > 0:
...
f = open('ngramErrorDetect.csv', 'w')
wr = csv.writer(f)

for lst in errorList:
    wr.writerow(lst)
